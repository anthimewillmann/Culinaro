...
final class RecipeAIService {
    ...
    func generate(from prompt: String, allergies: String = "") async throws -> ParsedRecipe {
        let session = LanguageModelSession(
            instructions: AILanguageResolver.instruction(for: .inputLanguageWithFallback(prompt))
        )
        var fullPrompt = "Create a complete recipe for the person's input below. Respond structured with title, ingredients and steps. Person's input: \(prompt)"
    ...
    func scan(image: UIImage) async throws -> ParsedRecipe {
        let session = LanguageModelSession(
            instructions: AILanguageResolver.instruction(for: .appLanguage)
        )
        let rawText = try await extractText(from: image)
        guard !rawText.isEmpty else { throw ScanError.noTextFound }
        let fullPrompt = "Extract recipe from: \(rawText)"
        let response = try await session.respond(to: fullPrompt, generating: ParsedRecipe.self)
        return response.content
    }
    ...
    func scanShoppingList(image: UIImage) async throws -> ParsedShoppingList {
        ...
        let rawText = try await extractText(from: image)
        guard !rawText.isEmpty else { throw ScanError.noTextFound }
        let fullPrompt = "Extract only the shopping list items or ingredients from this text. Keep visible quantities with the item, ignore notes, prices, totals and preparation instructions: \(rawText)"
    ...
    func estimateNutrition(title: String, ingredients: [String], steps: [String] = []) async throws -> NutritionEstimate {
        ...
        let fullPrompt = """
        Estimate approximate nutrition for this recipe or cooking lesson. This is only an estimate, not an exact medical or dietetic statement. Return all nutrition fields: calories, protein, carbohydrates and fat, plus the assumed number of servings. If ingredients are missing, infer a reasonable estimate from the title and preparation steps.
        Title: \(title)
        Ingredients:
        \(ingredientListing)
        Steps:
    ...
    func cookingTip(for step: String) async throws -> String {
        ...
        let fullPrompt = "Give me a very short, practical cooking tip for this step: \(step)"
    ...
    func generateLesson(from prompt: String) async throws -> ParsedLesson {
        ...
        let fullPrompt = "Create a short, beginner-friendly cooking lesson, similar to a Duolingo-style lesson, for the person's input below. Respond structured with a title, ingredients or tools used, and a sequence of teaching steps. Person's input: \(prompt)"
    ...
    func scanLesson(image: UIImage) async throws -> ParsedLesson {
        ...
        let rawText = try await extractText(from: image)
        guard !rawText.isEmpty else { throw ScanError.noTextFound }
        let fullPrompt = "Extract a cooking lesson from this text. Return the title, ingredients or tools used, and teaching steps: \(rawText)"
        let response = try await session.respond(to: fullPrompt, generating: ParsedLesson.self)
        return response.content
    }
    ...
    private func extractText(from image: UIImage) async throws -> String {
        ...
        return try await withCheckedThrowingContinuation { continuation in
            ...
            request.recognitionLanguages = AILanguageResolver.ocrRecognitionLanguages
